...
namespace LiteAbpUBD.Web.Areas.OpenApi
{
    ...
    public class OpenApiAuthorizeAttribute : Attribute, IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            var user = userManager.GetByIdAsync(apiKey).GetAwaiter().GetResult();
            ...
            var _sign = Common.ToolMethods.MD5Hash($"{apiSecret}{signStr}timestamp{tsStr}{apiSecret}".ToLower());
            ...
            var userPrincipal = signInManager.CreateUserPrincipalAsync(user).GetAwaiter().GetResult();
            context.HttpContext.User = userPrincipal;
            //验证权限
            var authorizationService = context.GetRequiredService<IAuthorizationService>();
            if (!authorizationService.IsGrantedAsync(PermissionName).GetAwaiter().GetResult())
            {
                if (env.IsDevelopment())
                    result.Content = "缺少授权";
                context.Result = result;
